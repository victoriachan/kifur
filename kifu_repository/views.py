...
def kifu_edit(request, kifu_id):
    k = get_object_or_404(Kifu, pk=kifu_id)
    return render_to_response('kifu_repository/kifu_edit.html', {'kifu': k})

def kifu_detail(request, kifu_id):
    k = get_object_or_404(Kifu, pk=kifu_id)
    return render_to_response('kifu_repository/kifu_detail.html', {'kifu': k})

def player_detail(request, player_id):
    p = get_object_or_404(Player, pk=player_id)
    return render_to_response('kifu_repository/player_detail.html', {'player': p})